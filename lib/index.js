...
function getStatsdApi ({ hostname, port, lynxOptions, transmit, prefix }) {
  ...
  statsdApi = {
    increment (metric) { send(lynx, key, metric, 'increment', transmit) },
    timing (metric, value) { send(lynx, key, metric, 'timing', transmit, value) }
  }
...
module.exports = function ({ client }) {
  ...
  const { value: config, error } = Joi.validate(opts, Schema)
  ...
  let statsd = getStatsdApi(config)
