...
let statsdApi
function getStatsdApi ({ hostname, port, lynxOptions, transmit, prefix }) {
  if (statsdApi) {
    return statsdApi
  }

  const lynx = new Lynx(hostname, port, lynxOptions)
  const sdc = new SDC({ host: hostname, port: port, tcp: true })

  const key = [prefix, Os.hostname()].filter(Boolean).join('.')

  function send (client, key, metric, method, transmit, value) {
    if (lynxOptions.tcp === true) {
      if (method === 'increment') {
        value = ''
  ...
  statsdApi = {
    increment (metric) { send(lynxOptions.tcp === true ? sdc : lynx, key, metric, 'increment', transmit) },
    timing (metric, value) { send(lynxOptions.tcp === true ? sdc : lynx, key, metric, 'timing', transmit, value) }
  }
